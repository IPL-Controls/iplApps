...
# Create two TIFF file saving plugin
...
NDFileTIFFConfigure("FileTIFF2", $(QSIZE), 0, "$(PORT)", 0)
dbLoadRecords("NDFileTIFF.template",  "P=$(PREFIX):,R=TIFF2:,PORT=FileTIFF2,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT)")
...
# Optional: Create NDFFT plugin (areaDetector-2-5 and above only)
NDFFTConfigure("FFT1", $(QSIZE), 0, "$(PORT)", 0, 8, 0, 0, 0)
dbLoadRecords("NDFFT.template", "P=$(PREFIX):,R=FFT1:,NAME=FFT1,PORT=FFT1,ADDR=0,TIMEOUT=1,NCHANS=$(NCHANS),NDARRAY_PORT=$(PORT)")

# Optional: create a corrections plugin
NDRemoveOutliersConfigure("RO1", $(QSIZE), 0, "$(PORT)", 0, 0, 0)
dbLoadRecords("NDRemoveOutliers.template", "P=$(PREFIX):,R=RO1:,PORT=RO1,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT)")
...
# Optional: load NDPluginBlur plugin
NDBlurConfigure("BLUR1", $(QSIZE), 0, "$(PORT)", 0, 0, 0, 0)
dbLoadRecords("NDBlur.template",  "P=$(PREFIX):,R=Blur1:, PORT=BLUR1,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT)")
set_requestfile_path("$(ADPLUGINBLUR)/BlurApp/Db")

# Optional: Create NDTimeSeries plugin (areaDetector-2-4 and above only)
NDTimeSeriesConfigure("TS1", $(QSIZE), 0, "$(PORT)", 0, 8, 0, 0, 0)
dbLoadRecords("NDTimeSeries.template", "P=$(PREFIX):,R=TS1:,NAME=TS1,PORT=TS1,ADDR=0,TIMEOUT=1,NCHANS=$(NCHANS),NDARRAY_PORT=$(PORT)")
